...
public class RKPointPin: UIView {
    ...
    required public init(size: CGSize = CGSize(width: 32, height: 64), pinTexture: UIImage? = nil, color: UIColor? = nil) {
    ...
    private var safeFrame: CGRect? {
        ...
        return CGRect(origin: CGPoint(x: arView.safeAreaInsets.left, y: arView.safeAreaInsets.top),
                      size: CGSize(
    ...
    private func updatePos() {
        ...
        guard let entity = self.targetEntity, entity.scene != nil,
                let projPoint = arView.project(entity.position(relativeTo: nil)) else {
        ...
        let toEntityDirection = entity.position(relativeTo: nil) - arView.cameraTransform.translation
    ...
    private func setPos(to point: CGPoint) {
        guard let safeFrame = self.safeFrame, let viewWidth = self.viewWidth, let viewHeight = self.viewHeight else {
            return
        }
        ...
        self.transform = CGAffineTransform(rotationAngle: atan2(point.y - viewHeight / 2, point.x - viewWidth / 2) - .pi / 2)
